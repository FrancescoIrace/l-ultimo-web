    ...
    return (
        <div className="min-h-screen bg-slate-100 max-w-md mx-auto p-4 pb-20">
            ...
            <motion.div
                ...
                style={{ backgroundColor: teamDetails.primary_color || '#3b82f6', borderColor: teamDetails.secondary_color || '#1e40af', color: teamDetails.secondary_color || '#ffffff' }}
            ...
            >
                ...
                <div className="text-center mb-4">
                    <div className="flex-row items-center justify-center gap-2 mb-2">
                        <h1 className={`text-3xl font-black ${getTextColorClass(teamDetails.primary_color || '#3b82f6')}`}>{teamDetails.name}</h1>
                    ...
                    {teamDetails.sport && teamDetails.sport.length > 0 ? (
                        teamDetails.sport.map((sport, index) => (
                            <span
                                key={index}
                                className="inline-flex items-center border border-slate-200 gap-2 mx-1 my-1 bg-blue-500 text-white px-3 py-1 rounded-full text-xs font-semibold :hover:transition-transform scale-100 hover:scale-102 transition-all cursor-pointer"
                            >
                                {sport}
                            </span>
                        ))
                    ) : (
                        <span className={`text-sm ${getTextColorClass(teamDetails.primary_color || '#3b82f6')}`}>Nessuno sport associato</span>
                    )}

                    <div className='bg-white rounded-xl p-3 mt-4 text-center text-slate-500'>
                        <p className="font-semibold mb-2 text-md text-slate-800">Descrizione:</p>
